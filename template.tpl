...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "ACM Site Tracking GTM",
...
[
  ...
  },
  {
    "type": "SELECT",
    "name": "viewedOrBought",
    "displayName": "Product viewed or bought",
    "macrosInSelect": true,
    "selectItems": [
      {
        "value": "viewed_",
...
if(data.advertiserId){
    ...
    if( data.productId && cookiesConsentRetargeting==true && ( data.viewedOrBought == "viewed_" || data.viewedOrBought == "bought_" ) ){
        pushPixels( urltemplate[ data.retargeting ].replace("<advId>",data.advertiserId).replace("<kwd>", data.viewedOrBought + data.productId )  );
    
    }else{
        pushPixels( urltemplate.none.replace("<advId>",data.advertiserId).replace("<kwd>", data.viewedOrBought + data.productId ) + "&userId=0" );
    }
...
Created on 10/5/2022, 12:33:43 PM
